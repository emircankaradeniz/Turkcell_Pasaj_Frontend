...
export default function UrunDetay() {
  ...
  const urun: Urun | undefined = mockUrunListesi.find(
    (u) => u.id.toString() === id
  );
  ...
  useEffect(() => {
    const kayitliYorumlar: Yorum[] = JSON.parse(localStorage.getItem("yorumlar") || "[]");
    const filtreli = kayitliYorumlar.filter((y) => y.urunId.toString() === id);
    setYorumlar(filtreli);
  }, [id]);

  const yorumEkle = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const tumYorumlar: Yorum[] = JSON.parse(localStorage.getItem("yorumlar") || "[]");
  ...
  const benzerUrunler = mockUrunListesi.filter(
    (u: Urun) => u.kategori === urun.kategori && u.id !== urun.id
  );
