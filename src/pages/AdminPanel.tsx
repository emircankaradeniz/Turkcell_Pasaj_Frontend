...
export default function AdminPanel() {
  ...
  const [duzenlenenUrunId, setDuzenlenenUrunId] = useState<number | null>(null);
  ...
  const urunEkle = (e: FormEvent) => {
    ...
    if (duzenlenenUrunId) {
      setUrunler((prev) =>
        prev.map((u) =>
          u.id === duzenlenenUrunId ? { ...form, id: u.id, fiyat: Number(form.fiyat) } : u
        )
      );
    ...
    setForm({ ad: "", aciklama: "", fiyat: 0, gorsel: "", kategori: "" });
  };
  ...
  let filtrelenmisUrunler = urunler
    ...
    .filter((u) => u.ad.toLowerCase().includes(aramaTerimi.toLowerCase()));
  ...
  const kategoriEkle = () => {
    ...
    setForm((prev) => ({ ...prev, kategori: yeniKategori.trim() }));
    setYeniKategori("");
  };
